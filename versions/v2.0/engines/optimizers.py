...
def _regularize_covariance(cov_matrix):
    """Symmetrise covariance and add a small ridge so solvers stay stable."""
    cov_values = np.asarray(cov_matrix.values if hasattr(cov_matrix, "values") else cov_matrix, dtype=float)
    cov_values = np.nan_to_num(cov_values, nan=0.0, posinf=0.0, neginf=0.0)
    cov_values = 0.5 * (cov_values + cov_values.T)
    diag_mean = float(np.mean(np.diag(cov_values))) if cov_values.size else 0.0
    ridge = max(diag_mean, 1e-8) * 1e-4
    cov_values = cov_values + np.eye(cov_values.shape[0]) * ridge
    return cov_values


def _coerce_previous_weights(previous_weights, n):
    """Normalise an optional previous-weight vector for tactical overlays."""
    if previous_weights is None:
        return None
    arr = np.asarray(previous_weights, dtype=float).reshape(-1)
...
def _rebalance_to_bounds(weights, min_w, max_w, tol=1e-10, max_iter=10):
    """Project a weight vector back to long-only capped bounds while preserving sum=1."""
    w = np.clip(np.asarray(weights, dtype=float).reshape(-1), min_w, max_w)
    n = w.size
    for _ in range(max_iter):
        ...
        if abs(gap) <= tol:
            break
        if gap > 0:
            free = np.where(w < max_w - tol)[0]
            if free.size == 0:
                break
            add = gap / free.size
            w[free] = np.minimum(max_w, w[free] + add)
        else:
            free = np.where(w > min_w + tol)[0]
            if free.size == 0:
                break
            sub = (-gap) / free.size
            w[free] = np.maximum(min_w, w[free] - sub)
    total = float(w.sum())
    if total > 1e-12:
        w = w / total
...
def _apply_rebalance_overlay(candidate, previous_weights, rebalance_ratio, min_w, max_w):
    """Blend a fresh optimum toward the previous portfolio for partial tactical moves."""
    prev = _coerce_previous_weights(previous_weights, len(candidate))
    ratio = float(np.clip(rebalance_ratio, 0.0, 1.0))
    if prev is None or ratio >= 0.999:
        return _rebalance_to_bounds(candidate, min_w, max_w)
    blended = prev + ratio * (np.asarray(candidate, dtype=float) - prev)
    return _rebalance_to_bounds(blended, min_w, max_w)
...
def utility_portfolio(
    ...
    if n * min_w > 1.0 + 1e-9:
        raise ValueError(f"Infeasible weight bounds: {n} assets × min_w={min_w:.4f} exceeds 100%.")
    w = cp.Variable(n)
    prev = _coerce_previous_weights(previous_weights, n)
    turnover_penalty = max(float(turnover_penalty), 0.0)
    cov_values = _regularize_covariance(cov_matrix)

    utility_expr = expected_returns.values @ w - 0.5 * cp.quad_form(w, cov_values)
    if prev is not None and turnover_penalty > 0:
        utility_expr = utility_expr - turnover_penalty * cp.norm1(w - prev)
    objective = cp.Maximize(utility_expr)
    ...
    if weights is None:
        ...
        return None
    weights = _apply_rebalance_overlay(weights, prev, rebalance_ratio, min_w, max_w)

    ann_ret  = expected_returns.values @ weights * 252
    ann_risk = np.sqrt(weights.T @ cov_values @ weights) * np.sqrt(252)
...
def compute_frontier(expected_returns, cov_matrix, min_w, max_w, n_points=100):
    ...
    for r_target in target_returns:
        ...
        prob = cp.Problem(cp.Minimize(cp.quad_form(w_m, cov_values)), constraints)
        prob.solve(solver=cp.OSQP)

        if w_m.value is None:
            continue
        wt   = w_m.value
        risk = np.sqrt(wt.T @ cov_values @ wt) * np.sqrt(252)
...
def tangency_portfolio(
    ...
    def _neg_sharpe_safe(w):
        ...
        if not np.isfinite(sharpe):
            return 1e9
        penalty = turnover_penalty * float(np.abs(w - prev).sum()) if prev is not None and turnover_penalty > 0 else 0.0
    ...
    for x0 in seeds:
        ...
        if not result.success:
            continue
        candidate = _apply_rebalance_overlay(result.x, prev, rebalance_ratio, min_w, max_w)
        total = candidate.sum()
...
def min_risk_portfolio(
    ...
    objective = cp.quad_form(w1, cov_values)
    if prev is not None and turnover_penalty > 0:
        objective = objective + turnover_penalty * cp.norm1(w1 - prev)
    cp.Problem(cp.Minimize(objective), constraints).solve(solver=cp.OSQP)
