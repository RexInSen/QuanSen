...
def dynamic_lookback(index_symbol, start_date, end_date):
    ...
    vol30 = float(returns.rolling(30).std().iloc[-1])
    vol250 = float(returns.rolling(250).std().iloc[-1])

    if vol250 <= 0 or np.isnan(vol250):
        vol_ratio = 1.0
    else:
        vol_ratio = vol30 / vol250
...
#adding automatic lookback
    ...
    print(f"Momentum regime → lookback={lookback} days | vol_ratio={regime_meta['vol_ratio']:.2f}")
    """
    Parameters
    ----------
