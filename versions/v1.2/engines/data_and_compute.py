...
try:
    ...
    _ENGINE_HAS_SHRINKAGE = True
except ImportError:
    # fallback if engine module missing
    SHRINKAGE_ALPHA = 0.70
    _ENGINE_HAS_SHRINKAGE = False
...
def load_data(tickers, start_date, end_date, alpha=None):
    """
    Self-contained data loader — always works regardless of engine version.

    Does its own yfinance download (handles MultiIndex correctly), then:
      - If new engine available: delegates to _engine_load_data for shrinkage
      - If old engine: applies shrinkage inline using the same formula
    Always returns a 6-tuple: tickers, returns, expected_returns, cov_matrix, raw_er, bm_sym
    """
    ...
    # ── Old engine fallback: do data loading entirely in the GUI ──
    import yfinance as _yf_inner

    def _trim_to_common_window(prices_frame, ordered_tickers):
        first_valid = {}
        last_valid = {}
        for ticker in ordered_tickers:
            series = prices_frame[ticker].dropna() if ticker in prices_frame.columns else pd.Series(dtype=float)
            if series.empty:
                continue
            first_valid[ticker] = series.index[0]
            last_valid[ticker] = series.index[-1]

        valid_tickers = [t for t in ordered_tickers if t in first_valid and t in last_valid]
        ...
        trimmed = prices_frame.loc[common_start:common_end, valid_tickers].copy()
        return trimmed, valid_tickers

    data = _yf_inner.download(
        tickers,
        start=start_date,
        end=end_date,
        auto_adjust=True,
    ...
    # Retry missing symbols one by one; some Yahoo symbols fail only in batch mode.
    for ticker in tickers:
        if ticker in prices.columns and prices[ticker].dropna().size > 0:
            continue
        try:
            single = _yf_inner.download(
                ticker,
                start=start_date,
                end=end_date,
                auto_adjust=True,
            ...
            if isinstance(single.columns, pd.MultiIndex):
                single_prices = single["Close"]
            else:
                single_prices = single[["Close"]] if "Close" in single.columns else single
            if isinstance(single_prices, pd.Series):
                single_prices = single_prices.to_frame(name=ticker)
            ...
            pass
        try:
            hist = _yf_inner.Ticker(ticker).history(
                start=start_date,
                end=end_date,
                auto_adjust=True,
            )
    ...
    tickers_out = [t for t in tickers if t in prices.columns]
    prices = prices[tickers_out].sort_index()
    prices, tickers_out = _trim_to_common_window(prices, tickers_out)

    # Fill small gaps inside the common overlap window.
    prices = prices.ffill()
    ...
    too_sparse = [t for t in tickers_out if valid_counts.get(t, 0) < 20]
    if too_sparse:
        tickers_out = [t for t in tickers_out if t not in too_sparse]
        prices = prices[tickers_out]
        returns = returns[tickers_out]
    ...
    # ── Inline shrinkage ──────────────────────────────────────────
    # Auto-detect benchmark the same way the new engine does
    indian = sum(1 for t in tickers_out if t.endswith(".NS") or t.endswith(".BO"))
    if indian / max(len(tickers_out), 1) >= 0.5:
        bm_candidates = ["^NSEI", "^BSESN"]
    else:
        bm_candidates = ["^GSPC", "^IXIC"]

    bm_sym   = None
    bm_mean  = None
    for sym in bm_candidates:
        try:
            bm_data = _yf_inner.download(sym, start=start_date, end=end_date,
            ...
            if len(bm_ret) < 20:
                continue
            bm_mean = float(bm_ret.mean())
            bm_sym  = sym
            break
        except Exception:
            continue

    if bm_mean is not None and alpha < 1.0:
...
@st.cache_data(ttl=3600, show_spinner=False)
def cached_compute_momentum(tickers, start_date, end_date, shrink_er, lookback, skip, beta,auto_lookback):
    ...
    return compute_momentum(
        ...
        lookback=lookback,
        skip=skip,
        beta=st.session_state.beta_regime,
...
@st.cache_data(ttl=3600, show_spinner=False)
def cached_utility_portfolio(expected_returns, cov_matrix, tickers, min_w, max_w):
...
@st.cache_data(ttl=3600, show_spinner=False)
def cached_tangency_portfolio(expected_returns, cov_matrix, tickers, min_w, max_w):
...
@st.cache_data(ttl=3600, show_spinner=False)
def cached_min_risk_portfolio(expected_returns, cov_matrix, tickers, target_return, min_w, max_w):
